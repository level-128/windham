...
void test_create_open_chain(char * device_){
	...
	action_create(device_, DEFAULT_DISK_ENC_MODE, key1, -1, 10000, -1, false, DEFAULT_BLOCK_SIZE);
	action_open(device_, "my_crypt_device", key1, master_key, -1, 0, 5, false, false, false, true, false, false);
	
	action_close("my_crypt_device");
	
	interactive_ask_new_key_test_key = "hello world2";
	action_addkey(device_, key1, master_key, -1, 40000, DBL_MAX, 3, 0, 1, false);
