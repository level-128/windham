   ...
   case NMOBJ_MAPPER_DEVSTAT_SUSP: {
      ...
      if (! is_dry_run) {
         ...
         create_crypt_mapping_from_disk_key(
            ...
            is_no_map_partition);
         print_warning(
            _("Device %s is unlocked and suspended. Don't forget to close it using \"Resume\" when appropriate."),
            device);
      } else {
         char uuid_str[37];
   ...
   if (strcmp(uninit_device, "TAB") == 0) {
      ...
      for (int i = 0; i < entity_count; i ++) {
         if (is_pid1) { // print a short summery to dmesg instead
            printk(
               "%s: Pass %hu, Device %s, To %s, Flag %u",
               windhamtab_file,
               entities[i].pass,
               entities[i].device,
               entities[i].to,
...
#define HAS_FLGI(x) entities[i].option_flags & (1 << x)
         ...
         init_device(
            entities[i].device,
            false /* do not create loop for windhamtab */,
            false,
            HAS_FLGI(NMOBJ_windhamtab_nofail),
            true,
            0,
            0);
         ...
         if (strcmp(entities[i].key, "ASK") == 0) {
            ...
            if (key.key_type != NMOBJ_key_file_type_key) {
               key.key_type = HAS_FLGI(NMOBJ_windhamtab_systemd)
         ...
         action_open(
            ...
            key,
            master_key,
            HAS_FLGI(NMOBJ_windhamtab_max_unlock_mem)
               ? entities[i].max_unlock_mem
               : max_unlock_mem,
            HAS_FLGI(NMOBJ_windhamtab_max_unlock_time)
               ? entities[i].max_unlock_time
               : max_unlock_time,
            0,
            false,
            false,
