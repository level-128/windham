...
bool read_GPT_header(
  ...
  return false;
   ...
   if (memcmp((uint8_t[sizeof(hdr.padding)]){0}, hdr.padding, sizeof(hdr.padding)) != 0 || hdr.__reserved != 0) {
...
#ifdef WINDHAM_ISOC
  print_warning(_("Cannot map device %s to %s. Not available under ISO C mode."), device, name);
