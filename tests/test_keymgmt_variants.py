...
_OPEN = lambda d, k="123": ["Open", d, f"--key={k}", "--max-unlock-time=1", "--dry-run", "--no-admin"]
...
def test_anonymous_key(binary, device):
    ...
    assert_success(["AddKey", device, "--key=123", "--target-time=0.1", "--rapid-add",
                    "--generate-random-key", "--anonymous-key", "--no-admin"], binary)
...
def test_del_key_make_anonymous(binary, device):
    ...
    assert_success(_NEW(device), binary)
    assert_success(["AddKey", device, "--key=123", "--target-time=0.1", "--rapid-add",
                    "--generate-random-key", "--no-admin"], binary)
    assert_success(["DelKey", device, "--key=123", "--yes", "--max-unlock-time=1", "--no-admin"], binary)
