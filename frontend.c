...
enum {
    ...
    ENUM_target_slot,
    ENUM_target_max_mem,
    ENUM_target_target_time,

    ENUM_target_dry_run,
    ENUM_target_readonly,
    ENUM_target_noadmin,
    ENUM_target_visible,
    ENUM_target_yes,
    ENUM_target_license,
...
int options[ENUM_target_SIZE] = {0};

struct option long_options[] = {
        {"map-to", required_argument, &options[0], 1},
        {"key", required_argument, &options[1], 1},
        {"key-file", required_argument, &options[2], 1},
        {"target-slot", required_argument, &options[3], 1},
        {"max-memory", required_argument, &options[4], 1},
        {"target-time", required_argument, &options[5], 1},

        {"dry-run", no_argument, &options[6], 1},
        {"readonly", no_argument, &options[7], 1},
        {"no-admin", no_argument, &options[8], 1},
        {"visible", no_argument, &options[9], 1},
        {"yes", no_argument, &options[10], 1},
        {"license", no_argument, &options[11], 1},
        {0, 0, 0, 0}
};
...
noreturn void frontend_help(const char * the_3rd_argv){
    if (!the_3rd_argv){
        ...
        print("Open <target>: open the target and create a mapper. The key, by default, is read from the terminal.\n"
              ...
              "\t--no-admin: forfeit checking for root privileges, may produce undefined behavior. ");
        ...
        print("Create <target>: create a "THE_NAME_OF_THIS_SOFTWARE" header on a block device and add a new key.\n"
              "\n"
              "options:\n"
              "\t--key <characters>: key input as argument, instead of asking in the terminal.\n"
              "\t--key-file <location>: key input as key file. The key file will be read as key (exclude EOF character). Option '--key' and '--key-file' are mutually exclusive.\n"
