...
- **辅助数据区** — 与密钥绑定的加密元数据：纯文本备注、Shell 命令（传入 `--aux-exec` 时才执行）、链接解锁条目。会随加密头重变换而保留。
...
- **文件形式加密容器** — `--diskfile` 创建以稀疏文件为存储介质的加密卷，支持 `--create-exfat` 自动格式化为 exFAT
- **离线解密** — 通过 `--decrypt --to=<文件>` 将解密内容输出为文件，无需 dm-crypt 或 root 权限
