...
- **大小**：创建时为 16 扇区 = 8 KiB。这是编译期常量（`windham_config.h` 中的
  `DEFAULT_AUX_SECTOR_SIZE`），没有按设备调整的命令行选项。
- **加密方式**：每个条目有自己独立的随机初始向量，用 AES-CBC 加密。密钥来自解锁所用密钥槽的口令哈希值。
- **公开条目**：若使用主密钥解锁，辅助数据区的槽密钥变为全零（不加密）。这类零密钥条目在**用主密钥解锁**时会被 `--aux-probe` 列出；用错误口令探测时连辅助数据区本身都无法读取。
...
```bash
windham Aux /dev/sda --aux-add="密码提示：哈基米曼波"
```
...
**标志位**：
- `BLCKOPEN`——若本命令执行成功，则阻止接下来的命令的执行。当一次 `Open` 被级联多个设备后
（详见接下来的 _连接解锁条目_），那么会阻止接下来的任何命令的执行，不光光是本设备辅助数据区
里的命令。
...
```
这个命令会将@替换为本次打开的设备，最后会执行 `mount /dev/mapper/mydisk /mnt/mymnt` 。
...
对一个含有链接解锁条目的设备执行 `Open` 时：

1. 父设备先解锁并建立映射。
2. 读取并解密其辅助数据区，扫描所有链接解锁条目。
3. 每个条目按优先级降序排列，然后以"当前深度 + 1"推入 FIFO 处理队列。
4. 队列依次处理每一个关联设备，而每个关联设备被打开后又可能再贡献出**自己的** 链接解锁条目，形成更深层的递归级联。
...
| 脚本选项 | 取值 | 说明 |
|---|---|---|
| `--raid=` | `raid1`（默认）、`raid5`、`raid6` | 链路拓扑 |
