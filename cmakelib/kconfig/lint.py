...
def check_defconfig_only_definition():
    print_header("Symbols only defined in Kconfig.other_defconfig files")
    for sym in kconf.unique_defined_syms:
        if all("other_defconfig" in node.filename for node in sym.nodes):
            print(name_and_locs(sym))
