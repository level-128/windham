...
class Kconfig(object):
    ...
    modules:
      ...
      nearly all other_defconfig files enable them (with 'CONFIG_MODULES=y').

    defconfig_list:
      The Symbol instance for the 'option defconfig_list' symbol, or None if no
      defconfig_list symbol exists. The other_defconfig filename derived from this
    ...
    defconfig_filename:
      ...
      exists (can be opened for reading). If a other_defconfig file foo/other_defconfig is
      not found and $srctree was set when the Kconfig was created,
      $srctree/foo/other_defconfig is looked up as well.
      ...
      Gotcha: scripts/kconfig/Makefile might pass --other_defconfig=<other_defconfig> to
      scripts/kconfig/conf when running e.g. 'make other_defconfig'. This option
      overrides the defconfig_list symbol, meaning defconfig_filename might not
      always match what 'make other_defconfig' would use.
